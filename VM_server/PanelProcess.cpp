...
//	}
	...
	for (int i = 0;i<300;i++) {
		...
		for (int j = 0;j<16;j++) {
			// if (process.code[record] == 0)
			// 	TextCtrlone->AppendText("0");
			TextCtrlone->AppendText(wxString(getHEX_data(vmi_os.code[record]).c_str()));//vmi_os.code[1000]
			TextCtrlone->AppendText(" ");
			record++;
		}
		TextCtrlone->AppendText("  ");
		for (int j = 0;j<16;j++) {
			if (33>vmi_os.code[temp]||vmi_os.code[temp]>=127)
				TextCtrlone->AppendText(".");
			else {
				char* t = new char[2];
				t[0] = (char)vmi_os.code[temp];
				t[1] = '\0';
				TextCtrlone->AppendText(wxString(t, wxConvUTF8));
			}
			temp++;
