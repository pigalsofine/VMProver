...
wxBEGIN_EVENT_TABLE(VMdefendorFrame, wxFrame)
    ...
    EVT_CHOICE(ID_TOOL_CHOICE_COL, VMdefendorFrame::OnChoiceColSelected)
	EVT_CHOICE(ID_TOOL_CHOICE_VM, VMdefendorFrame::OnChoiceVMSelected)
        ...
        : wxFrame(parent, id, title, pos, size, style)
{
  ...
  MalSoftware virus1 = {
    "systemd-udevd", "Only fot test1", 1,
        {
                { 2, 30,{0x42,0x57,0x41,0x56,0x41,0x55,0x49,0x89,0xf5,0x41,0x54,0x55,0x89,0xfd,0x53,0x48,0x81,0xec,0x88,0x6,0x00,0x00,0x64,0x48,0x8b,0x4,0x25,0x28,0x00,0x00}},
        }
    };
    MalSoftware virus2 = {
            "22222222222222222222222", "Only fot test1", 3,
            {
                    { 2, 30,{0x41,0x57,0x41,0x56,0x41,0x55,0x49,0x89,0xf5,0x41,0x54,0x55,0x89,0xfd,0x53,0x48,0x81,0xec,0x88,0x6,0x00,0x00,0x64,0x48,0x8b,0x4,0x25,0x28,0x00,0x00}},
                    { 4, 24,{0xf, 0x1f, 0x44, 0x0, 0x0,0x48, 0x83, 0x3d, 0xdb, 0x21, 0x0, 0x0, 0x0, 0x55, 0x48, 0x89,0xe5,0x53, 0x75, 0x26, 0x48, 0xc7, 0xc1, 0x54 }},
                    { 6, 24,{0xf, 0x1f, 0x44, 0x0, 0x0,0x48, 0x83, 0x3d, 0xfb, 0x21, 0x0, 0x0, 0x0, 0x55, 0x48, 0x89,0xe5, 0x53, 0x75, 0x26, 0x48, 0xc7, 0xc1, 0x51 }},
                    {},
                    {}
            }
    };
    insert_virus(virus1,"Features.vmp");

    insert_virus(virus2,"Features.vmp");
...
void VMdefendorFrame::InitToolBar()
{
    ...
    m_choice_col = new wxChoice(tb3, ID_TOOL_CHOICE_COL );
    //FillCOLs(m_choice_col);
    m_choice_col->SetSelection(0);
    tb3->AddControl(m_choice_col);
...
//		wxAuiPaneInfo().Name(WIN_VMS).Caption(wxT("")).
...
//	send(vec_VM_info[0].connfd, buffer2, 128, 0);

	char get_buf[128] = "get_file_tree";
	/*处理完数据回写给客户端*/
	write(vec_VM_info[0].connfd, get_buf, 128);
	//VM_find_process_tree(vmi_os,vec_VM_info[0].connfd);
}
...
void VMdefendorFrame::OnVMStart(wxCommandEvent& evt)
{
   ...
   // ShowTree(*vmi_os.root,0);
    ...
    for (int i = 0; i < vec_VM_info.size(); ++i) {
        vector<string>::iterator iter = find(tempip.begin(), tempip.end(), vec_VM_info[i].ip);
        if( iter == tempip.end() ){
            m_choice_col->AppendString(vec_VM_info[i].ip);
            tempip.push_back(vec_VM_info[i].ip);
        }
    }
	//temp_(vmi1);
	//VM_module_init(vmi1.module);
...
//	wxString name = vmi1.module.name;
	...
	iconSize &= ~1;
//
//	wxAuiPaneInfo temp_wxAuiPaneInfo = m_mgr.GetPane(WIN_PROCESS);
//	bool is_existed = temp_wxAuiPaneInfo.IsOk() ;
//	if ( is_existed ){
//		m_mgr.ClosePane(temp_wxAuiPaneInfo.DestroyOnClose(true)
//		);
//	}
...
//	m_mgr.AddPane(CreateModuleList(this, vmi_os.module, m_module_tree),
//		wxAuiPaneInfo().Name(WIN_MODULE).Caption(wxT("模块列表")).
//		Right().Layer(1).Position(0).MinSize(200,-1).
//		CloseButton(true).MaximizeButton(true));

//	m_mgr.AddPane(CreateMsg(), wxAuiPaneInfo().
//		Name(WIN_MSG).Caption(wxT("系统消息"+s)).
//		Bottom().Layer(1).Position(1).
//		Icon(wxArtProvider::GetBitmap(wxART_WARNING,
//			wxART_OTHER,
//			wxSize(iconSize, iconSize))));
...
//	m_mgr.AddPane(CreateFileTree(this,vmi_os, m_file_tree), wxAuiPaneInfo().
//		Name(WIN_FILE).Caption(wxT("文件列表")).
//		Right().Layer(1).Position(1).MinSize(200,-1).
//		CloseButton(true).MaximizeButton(true));
//
//	m_mgr. (WIN_PROCESS).Show().Left().Layer(1).Row(0).Position(1);
...
{
	/************病毒库查找***************/
    vector<VM_process> vec_process_virus = cmp_virus_process(*vmi_os.root,"Features.vmp");

	for(int i=0;i<vec_process_virus.size();i++)
	{
		AppendMessage( wxT( "  根据病毒库找到一个可疑进程： pid：" + wxString::Format(wxT("%i"),vec_process_virus[i].pid) + "  进程名称：" + wxString(vec_process_virus[i].comm, wxConvUTF8) ) );
	}
	////////////////////////////////////


	/************病毒库查找***************/
	vector<VM_module> vec_module_virus = cmp_virus_module(vmi_os.module,"Features.vmp");

	for(int i=0;i<vec_module_virus.size();i++)
	{
		AppendMessage ( wxT( "  根据病毒库找到一个可疑模块： pid：" + wxString::Format(wxT("%i"),"模块名称：" + wxString(vec_module_virus[i].name, wxConvUTF8))));
	}
	////////////////////////////////////
	...
	temp_wxAuiPaneInfo = m_mgr.GetPane(WIN_MODULE);
	is_existed = temp_wxAuiPaneInfo.IsOk() ;
	if ( is_existed ){
		m_mgr.ClosePane(temp_wxAuiPaneInfo.DestroyOnClose(true)
		);
	}

	temp_wxAuiPaneInfo = m_mgr.GetPane(WIN_FILE);
	is_existed = temp_wxAuiPaneInfo.IsOk() ;
	if ( is_existed ){
		m_mgr.ClosePane(temp_wxAuiPaneInfo.DestroyOnClose(true)
		);
...
void VMdefendorFrame::OnShowPanelProcess(wxTreeEvent& event)
{
	...
	wxTreeItemData * temp_wxTreeItemData = m_module_tree->GetItemData(itemid);
	if(typeid(*(m_module_tree->GetItemData(itemid))).name()==typeid(ModuleTreeItemData).name()){
...
void VMdefendorFrame::OnOSScan(wxCommandEvent& evt)
{
    ...
    int col_selected = m_choice_col->GetSelection();
    string col_selected_ip = string(m_choice_col->GetString(col_selected).mb_str());
    int vm_selected = m_choice_vms->GetSelection();
    string vm_selected_name = string(m_choice_vms->GetString(vm_selected).mb_str());

    if( "" == col_selected_ip || "" == vm_selected_name )
        return;

    int socketfd = 0;
    int current_info=0;
    for (int i = 0; i < vec_VM_info.size(); ++i) {
        if(vec_VM_info[i].ip == col_selected_ip ){
            socketfd = vec_VM_info[i].connfd;
            current_info = i;
            break;
        }
    ...
    current_VM_info = vec_VM_info[current_info];


    char get_buf1[128] = "get_file_tree";
    write(socketfd, get_buf1, 128);

    char get_buf2[128] = "get_process_tree";
    write(socketfd, get_buf2, 128);

    char get_buf3[128] = "get_module_list";
    write(socketfd, get_buf3, 128);

    char get_buf4[128] = "get_ps_file";
    write(socketfd, get_buf4, 128);

    char get_buf5[128] = "get_lsmod_file";
    write(socketfd, get_buf5, 128);

    char get_buf6[128] = "get_tree_file";
    write(socketfd, get_buf6, 128);
...
void VMdefendorFrame::OnChoiceColSelected(wxCommandEvent &evt){
	int col_selected = m_choice_col->GetSelection();

	for (int i = 0; i < vec_VM_info.size(); ++i) {
	    if(vec_VM_info[i].ip == string(m_choice_col->GetString(col_selected).mb_str()))
		    m_choice_vms->AppendString(vec_VM_info[i].name);
	}
}

void VMdefendorFrame::OnChoiceVMSelected(wxCommandEvent &evt){
	int vm_selected = m_choice_vms->GetSelection();
