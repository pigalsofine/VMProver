...
void get_module_info(VM_vmi &vmi_os,VM_module &module,addr_t current_module)
{
	...
	char * str_version = vmi_read_str_va(vmi_os.vmi,temp_address, 0);
    if (str_version != NULL )
	    memcpy(module.version, str_version ,strlen(str_version));
    else
        memcpy(module.version, "" ,strlen(""));
	//if(module.version!=NULL)
	//	printf("%s\n",module.version);

	vmi_read_64_va(vmi_os.vmi, current_module +0xb8, 0, &temp_address);//srcverision
	char * s_srcversion = vmi_read_str_va(vmi_os.vmi,temp_address, 0);
    if (s_srcversion != NULL)
	    memcpy(module.srcversion, s_srcversion ,strlen(s_srcversion));
    else
        memcpy(module.srcversion, "" ,strlen(""));
	//if(module.srcversion!=NULL)
	//	printf("%s\n",module.srcversion);

	vmi_read_64_va(vmi_os.vmi, current_module +0x2a8, 0, &temp_address);//args
	char * s_args = vmi_read_str_va(vmi_os.vmi,temp_address, 0);
    if (s_args != NULL)
	    memcpy(module.args, s_args ,strlen(s_args));
    else
        memcpy(module.args, "" ,strlen(""));
	//if(module.args!=NULL)
	//	printf("%s\n",module.args);
	...
	//	printf("````````````````%s\n",module.rootkid->name);
	 char *str_name = vmi_read_str_va(vmi_os.vmi,current_module + 0x10, 0);
    if (str_name != NULL)
        memcpy(module.name, str_name ,strlen(str_name));
    else
        memcpy(module.name, "" ,strlen(""));
	printf("name = %s\n",module.name);
