...
{//	printf("VM_find_process_code in\n");
    ...
    uint64_t real_start_code;
    vmi_read_64_va(vmi_os.vmi, process.start_code_address+0x18, process.pid, &real_start_code);
  //  printf("%lx\n", real_start_code);

    uint32_t temp0;
    vmi_read_32_va(vmi_os.vmi, real_start_code+0x4006fd-0x4006e0+3, process.pid, &temp0);
    printf("%d   %s   %lx     %lx\n",process.pid,process.comm, process.start_code_address,temp0);

	int k=0;
    //string asm_code;
	for (unsigned long i = temp0; i < temp0+300; i+=1){
		uint8_t temp=0;
		vmi_read_8_va(vmi_os.vmi, (addr_t)i, process.pid, &temp);
        process.code[process.code_long++] = temp;
        printf("%lx ",temp );
...
void VM_find_process_code_byaddress(uint8_t code[],uint64_t start,uint64_t end,VM_vmi vmi_os,int pid){
    cout<<"start "<<start<<" end "<<end<<" pid "<<pid<<"\n";
    for (unsigned long i = start ,k =0; i < end; i+=1){
        uint8_t temp=0;
        vmi_read_8_va(vmi_os.vmi, (addr_t)i, pid, &temp);
        code[k++] = temp;
        printf("%lx ",temp );
    }printf("\n");
}
...
{//printf("get_process_infornmation in\n");
	...
	char * s = vmi_read_str_va(vmi_os.vmi, current_process + vmi_os.name_offset, 0);//获取当前进程name
	memcpy(process.comm,s,strlen(s)); 
	 ...
	 for(t=next_list_entry,i=0;t!=current_process+vmi_os.children_offset;vmi_read_addr_va(vmi_os.vmi, t, 0, &t)){//循环sibling
	   ...
	   l = t-vmi_os.sibling_offset; //得到进程首地址
		vmi_read_32_va(vmi_os.vmi, l + vmi_os.pid_offset, 0, &vmi_os.pid);
         process.children_pid[process.num_children_pid++] = vmi_os.pid;
		if (0==vmi_os.pid)
		{
			break;
		}
...
{
printf("in VM_find_process_list\n");
  //  printf("********************************************打印链表进程**************************************\n");
...
printf("out VM_find_process_list\n");
